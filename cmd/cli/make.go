...
func doMake(arg2 string, arg3 string) error {
	...
	switch arg2 {
	case "key":
		rnd := ug.GenerateRandomString(32)
	...
	case "model":
		if arg3 == "" {
			exitGracefully(errors.New("you must provide a name for your model"))
		}
		...
		fileName := ug.RootPath + "/data/" + strings.ToLower(modelName) + ".go"
		if fileExists(fileName) {
			exitGracefully(errors.New("model already exists: " + fileName))
		}
	...
	case "handler":
		if arg3 == "" {
			exitGracefully(errors.New("you must provide a name for your handler"))
		}
		fileName := ug.RootPath + "/handlers/" + strings.ToLower(arg3) + ".go"
		if fileExists(fileName) {
	...
	case "mail":
		...
		htmlMail := ug.RootPath + "/mail/" + strings.ToLower(arg3) + ".html.tmpl"
		plainMail := ug.RootPath + "/mail/" + strings.ToLower(arg3) + ".plain.txt"
		...
		err = copyFileFromTemplate("templates/mailer/plain.html.txt", plainMail)
		if err != nil {
			exitGracefully(err)
		}
	default:
